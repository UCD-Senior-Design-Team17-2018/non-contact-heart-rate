            ...
            if frame_num >= 900: # when 900 frames collected, start calculating heart rate (sliding window)
                if (frame_num-900) % 1 == 0:
                  ...
                  #  X_f = signal.lfilter(b, a, X_f) 
                    ...
                    plt.gcf().clear()
                    fft_plot = yplot
                    lags = plt.xcorr(G_std,G_std,maxlags=30*25)
                    xflags = fftshift(fftfreq(len(lags[1]),T))
                    fft_plotlags = fftshift(abs(fft(signal.lfilter(b, a,lags[1]))))
                    # Find highest peak between 0.75 and 4 Hz 
                 #   fft_plot[xf<=0.75] = 0 
                    if frame_num == 900:
                        bpm = xflags[(xflags>=0.75) & (xflags<=4)][fft_plotlags[(xflags>=0.75) & (xflags<=4)].argmax()]*60
                    else:
                        bpm = 0*bpm + 1*(xflags[(xflags>=0.75) & (xflags<=4)][fft_plotlags[(xflags>=0.75) & (xflags<=4)].argmax()]*60)
