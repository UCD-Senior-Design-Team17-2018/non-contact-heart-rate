# -*- coding: utf-8 -*-
"""
Created on Sat May 12 02:52:36 2018
...
hamming = signal.firwin(100, [0.7,3], window = 'hamming', pass_zero=False,fs=20) #fs needs to be changed, doing in dark environment so...
...
while fps._numFrames < 1000: 
    try:
        frame = cap.read() # read in the frame, status
        ret = True
    except:
        ret = False
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            if frame_num >= 200: # when 900 frames collected, start calculating heart rate (sliding window)
                if (frame_num-200) % 1 == 0: # after every 1 frame, calculate heart rate using the data in the sliding window
                    length = 200 # about 25 seconds of data
                        #normalize RGB signals
                    G_std = signal.detrend(G[-length:-1])
                    
                    T = 1/(len(time[-(length-1):])/(time[-1]-time[-(length-1)])) #calculate time between first and last frame (period)
                    #print(1/T)
                    #T = 1/30 #period
